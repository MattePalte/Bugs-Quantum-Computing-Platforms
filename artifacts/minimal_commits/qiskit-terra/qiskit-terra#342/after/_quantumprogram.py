            ...
            if not backend_conf['simulator']:
                ...
                for i, instruction in enumerate(circuit.data):
                    if isinstance(instruction, Measure):
                        ...
                        qasm_idx.append(i)
                    elif isinstance(instruction, Gate) and bool(set(instruction.arg) & set(measured_qubits)):
                        raise QISKitError('backend "{0}" rejects gate after measurement in circuit "{1}"'.format(backend, name))
